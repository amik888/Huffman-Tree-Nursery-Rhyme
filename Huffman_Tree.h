...
public:
	...
	void assign_code(std::ostream& out)
	{
		assign_code(out, "", huff_tree);
	}

private:
	template<typename T>
	void assign_code(std::ostream& out, std::string code, const Binary_Tree<Huff_Data<T> >& tree)
	{
		...
		{
			assign_code(out, code + "0", tree.get_left_subtree());
			assign_code(out, code + "1", tree.get_right_subtree());
		}
	}
